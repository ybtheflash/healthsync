  ...
  try {
    const response = await GroqAPI.generateChatCompletion({
      ...
      messages: [
        { 
          role: "system", 
          content: "You are a medical query classifier. Categorize the following query accurately."
        },
        { 
          role: "user", 
          content: `Classify the following medical query into one of these categories:
...
async function verifyFactualClaims(response: string, query: string): Promise<{ verifiedResponse: string, sources: Source[] }> {
  try {
    ...
    const claimsResponse = await GroqAPI.generateChatCompletion({
      model: "deepseek-r1-distill-qwen-32b",
      messages: [
        {
          role: "system",
          content: "You are a medical fact-checker. Extract key factual claims from medical content."
        },
        {
          role: "user",
          content: `Extract 3-5 key factual claims from this medical response:
    ...
    const claims = claimsResponse.choices[0].message.content.trim().split('\n').filter(claim => claim.trim() !== '');
      ...
      if (searchResults.results && searchResults.results.length > 0) {
        ...
        const correctionCheck = await GroqAPI.generateChatCompletion({
          ...
          messages: [
            { 
              role: "system", 
              content: "You are a medical fact-checker. Review the claim against the search results and determine if any correction is needed." 
            },
            { 
              role: "user", 
              content: `Claim: "${claim}"
        ...
        if (correctionResult.toLowerCase().includes("no") && correctionResult.toLowerCase().includes("correction")) {
          const correction = await GroqAPI.generateChatCompletion({
            ...
            messages: [
              { 
                role: "system", 
                content: "You are an expert at making minimal, precise corrections to text while preserving the surrounding content." 
              },
              { 
                role: "user", 
...
export async function analyzeMedicalQuery(query: string): Promise<PulseResponse | PulseError> {
  try {
    ...
    return {
      message: "Our servers are currently busy processing other medical queries. Please try again in a few moments.",
...
export async function analyzeMedicalDocument(documentContent: string): Promise<PulseResponse | PulseError> {
  try {
    ...
    return {
      message: "Our document analysis servers are currently busy. Please try uploading your document again in a few moments.",
